...
__device__ glm::vec3 computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* svec, const float* shs, bool* clamped)
{
	...
	glm::vec3 sun(svec[0],svec[1],svec[2]);
	// printf("sun = (%f, %f, %f)\n", sun.x, sun.y, sun.z);
	...
	bool prefiltered)
{
	...
	if (colors_precomp == nullptr) {
		glm::vec3 result = computeColorFromSH(idx, D, M, (glm::vec3*)orig_points, *cam_pos, svec, shs, clamped);
